    ...
    Formato de data/perfil/bibliotecario/book-pipeline-config.json:
      {
        ...
        "categorias_ignoradas": ["Nome de categoria a pular", ...]
      }
    Sem o arquivo: drive_root_folder_id vazio (get_inventario avisa e para) e mapeamento
    vazio (tudo cai em "_to_organize", fallback seguro que nunca perde material).
    """
    if not CONFIG_PATH.exists():
        return {'drive_root_folder_id': None, 'mapeamento': {}, 'categorias_ignoradas': []}
    try:
        with open(CONFIG_PATH) as f:
            cfg = json.load(f)
    except Exception as e:
        log.warning(f'config em {CONFIG_PATH} inválida ({e}), usando fallback vazio')
        return {'drive_root_folder_id': None, 'mapeamento': {}, 'categorias_ignoradas': []}
    return {
        'drive_root_folder_id': cfg.get('drive_root_folder_id'),
        'mapeamento': {cat: tuple(dest) for cat, dest in cfg.get('mapeamento', {}).items()},
        'categorias_ignoradas': cfg.get('categorias_ignoradas', []),
    }

_CONFIG = carregar_config()
DRIVE_ROOT_FOLDER_ID = _CONFIG['drive_root_folder_id']
MAPEAMENTO = _CONFIG['mapeamento']
CATEGORIAS_IGNORADAS = set(_CONFIG['categorias_ignoradas'])

def get_destino(item):
    """Decide destino do arquivo. Sem categoria mapeada -> _to_organize (usuário reclassifica depois)."""
    cat = item['categoria_drive']
    return MAPEAMENTO.get(cat, ('_to_organize', 'XX-'))
...
def get_inventario():
    if not DRIVE_ROOT_FOLDER_ID:
        raise RuntimeError(
            f'drive_root_folder_id ausente em {CONFIG_PATH} — veja SKILL.md ("Configuração obrigatória")'
        )
    cats = api('files', {
        'q': f"'{DRIVE_ROOT_FOLDER_ID}' in parents and trashed=false and mimeType='application/vnd.google-apps.folder'",
        'fields': 'files(id,name)',
        'pageSize': 100,
    }).get('files', [])
    inventário = []
    for c in cats:
        if c['name'] in CATEGORIAS_IGNORADAS:
            continue
...
def converter(input_path, output_path, ext='', timeout_min=10):
    ...
    if ext == 'pdf':
        ...
        try:
            r = subprocess.run(
                ['python3', str(OCR_SCRIPT), str(input_path), str(output_path)],
