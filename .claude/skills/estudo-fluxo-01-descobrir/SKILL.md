...
4. Sugere estrutura de estudo — desenhar como mapa/fluxograma com a skill `mermaid-diagrams`
   (`.claude/skills/mermaid-diagrams/`) ajuda o aluno a visualizar antes de começar. Se o tópico tem
   vários conceitos inter-relacionados (não é um assunto isolado), use `study-concept-mapping` pra
   gerar o mapa real (Mermaid + clusters) em vez de desenhar à mão; se a estrutura for principalmente
   de pré-requisitos em sequência, use `study-knowledge-graph` pra calcular o caminho de estudo
