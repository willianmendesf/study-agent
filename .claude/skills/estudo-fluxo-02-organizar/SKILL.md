...
3. Extrai conceitos-chave do `.md` gerado (nunca do arquivo original). Se a matéria tem pré-requisitos
   claros entre os conceitos extraídos, registra/atualiza `data/knowledge-graphs/<materia>.yaml`
   (`study-knowledge-graph`) e/ou `data/concept-maps/<materia>.yaml` (`study-concept-mapping`) —
   não força se a matéria não tiver essa estrutura
